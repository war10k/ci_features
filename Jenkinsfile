...
pipeline {
    ...
    stages {
        stage("Инициализация переменных") {
            steps {
                script {
                    ...
                    SMOKE_TEST_REPORT = "${CURRENT_CATALOG}\\Reports\\resultopenform.xlsx"
        ...
        stage('XUnit test Открытие форм') {
            steps {
                script {
                    cmd("\"${PLATFORM_1C}\" ENTERPRISE /F \"${PROJECT_IB_CATALOG}\" /DisableStartupMessages /DisableStartupDialogs /C\"xddConfig \"\"${SMOKE_TEST_JSON}\"\" ; xddRun ЗагрузчикФайла \"\"${SMOKE_TEST}\"\"\\Тесты_ОткрытиеФормКонфигурации.epf; xddReport ОтчетОтсутствиеОшибокMXL \"\"${SMOKE_TEST_REPORT}\"\"; xddShutdown\" /TESTMANAGER /Execute \"${TEST_RUNNER}\"")
                }
            }
        }
    }
